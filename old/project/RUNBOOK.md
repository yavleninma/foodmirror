...
## Остановка всех Docker-проектов на droplet

Если на сервере крутятся старые проекты и нужно начать с чистого листа:

```bash
ssh root@157.230.246.177

# 1. Посмотреть, что запущено
docker ps -a

# 2. Остановить все контейнеры
docker stop $(docker ps -q)

# 3. (Опционально) Остановить и удалить контейнеры, сети, volumes по всем compose-проектам
# Если проекты в разных папках — зайти в каждую и:
cd /opt/foodmirror   # или путь к другому проекту
docker compose down

# 4. (Опционально) Удалить неиспользуемые образы, чтобы освободить место
docker image prune -a
```

**Рекомендация:** сначала `docker stop $(docker ps -q)`, потом в `/opt/foodmirror` выполнить `docker compose up -d postgres` (подождать), затем `deploy.sh` или шаги из «Первый деплой» по необходимости.
