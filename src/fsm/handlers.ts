...
export async function handleMessage(
  ...
  if (msg.text?.startsWith("/start") || msg.text === HELP_BUTTON) {
    const startText = [
      "FoodMirror — бот для фиксации питания: отправляешь фото еды или текст, получаешь оценку калорий и БЖУ (с диапазоном). Никаких целей и советов — только факт дня.",
      "",
      "Как пользоваться: отправь фото или текст → проверяешь оценку → подтверждаешь. Без подтверждения ничего не сохраняется.",
      "",
      "Триал 14 дней — полный доступ. Дальше бесплатно: оценка без лимитов; итоги за день и статистика — в Premium.",
    ].join("\n");
    await sendLoggedMessage(bot, {
      userId: user.id,
      chatId,
  ...
  if (msg.text?.startsWith("/") && msg.text.length > 1) {
    const helpText = [
      "Такой команды нет. Всё нужное — в кнопках меню внизу: Помощь, Условия, Поддержка, Напоминание.",
    ].join("\n");
    await sendLoggedMessage(bot, {
      userId: user.id,
      chatId,
